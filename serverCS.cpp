...
void loadCourseCatalogue(std::map<std::string, std::vector<std::string> > &cs_catalogue){
    ...
    while(fgets(buf, sizeof(buf), fp) != NULL){
        ...
        cs_catalogue[courseCode]=loadCourseData(courseData, sizeof(courseData));
    }
}
...
std::string processCourseCatalogue(std::map<std::string, std::vector<std::string> > &map_cs, char *data, const int len){
    char courseCode[COURSE_CODE_LENGTH+1];
    ...
    std::map<std::string, std::vector<std::string> >::iterator it = map_cs.find(std::string(courseCode));
    if(it == map_cs.end()){
    ...
    std::vector<std::string> courseInfo = map_cs[std::string(courseCode)];
...
int main(void)
{
    ...
    std::map<std::string, std::vector<std::string> > cs_catalogue;
...
/*
    for(std::map<std::string, std::vector<std::string> >::iterator it=cs_catalogue.begin(); it!=cs_catalogue.end(); ++it){
...
*/
    while(1){
        ...
        // 2. Process the data
        std::string info = processCourseCatalogue(cs_catalogue, buf, strlen(buf));
